...
class IDEC(nn.Module):
    def __init__(self,
                 ...
                 n_clusters,
                 alpha=1,
                 pretrain_path='data/ae_mnist.pkl' ,device="cpu"):
        ...
        self.p_cluster_layer = Parameter(torch.Tensor(n_clusters, n_z))
        self.n_cluster_layer = Parameter(torch.Tensor(n_clusters, n_z))
    ...
    def pretrain(self, train_loader, path=''):
        ...
        if not is_non_zero_file(path):
            path = ''
        if path == '':
            pretrain_ae(self.ae, train_loader, self.device)
        else:
